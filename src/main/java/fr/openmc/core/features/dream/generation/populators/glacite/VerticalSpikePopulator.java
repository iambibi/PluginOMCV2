...
public class VerticalSpikePopulator extends BlockPopulator {
    private static final double CHUNK_VERTICAL_SPIKE_PROBABILITY = 0.6;

    private static final double PER_SOL_PROBABILITY = 0.01;

    private static final List<String> VERTICAL_SPIKE_FEATURES = new ArrayList<>(List.of(
    ...
    public void populate(@NotNull World world, @NotNull Random random, @NotNull Chunk chunk) {
        if (random.nextDouble() >= CHUNK_VERTICAL_SPIKE_PROBABILITY) return;
                                    ...
                                    try {
                                        StructureUtils.placeStructure(
                                                StructureUtils.getStructureFile("omc_dream",
                                                        VERTICAL_SPIKE_FEATURES.get(random.nextInt(VERTICAL_SPIKE_FEATURES.size()))),
                                                loc,
                                                random.nextBoolean(),
                                                random.nextBoolean()
                                        );
